...
class defaultCtrl extends jControllerDaoCrud {
     ...
     * method to return a personalized response
    ...
    protected function _checkData($form, $calltype)
    {
        ...
        if ($calltype) {
            $rec = $dao->getBySlugExceptId($form->getData('slugurl'), $this->param('id'));
        }
        else {
            $rec = $dao->getBySlug($form->getData('slugurl'));
        }

        if ($rec) {
            $form->setErrorOn('slugurl', jLocale::get('news_admin~news.form.slugurl.already.exists'));
            return false;
        }
    ...
    /**
     * redefine this method if you want to do additional during the precreate action
    ...
    /**
     * redefine this method if you want to do additional things on the response and on the edit template
    ...
    {
        $resp->addJSLink(jUrl::get('news_admin~www', ['file' => 'newsadmin.js']));
        if (class_exists('jAuthentication')) {
            $user = jAuthentication::getCurrentUser();
            if ($user) {
                $form->setData('author', $user->getName());
            }
        }
    }

    /**
     * redefine this method if you want to do additional things on the dao generated by the
    ...
    /**
     * redefine this method if you want to do additional things after the creation of
    ...
    /**
     * redefine this method if you want to do additional things during preupdate action
     * @param jFormsBase $form the form object
     */
    ...
    /**
     * redefine this method if you want to do additional things after the update of
     * a record. For example, you can handle here the uploaded files. If you do
     * such handling, set the uploadsDirectory property to false, to prevent
     * the default behavior on uploaded files in the controller.
     * @param jFormsBase $form the form object
    ...
    /**
     * redefine this method if you want to do additional things on the response and on the view template
    ...
    /**
     * @return jResponse|jResponseBinary
     */
    public function www()
    {
        $filename = realpath(__DIR__.'/../www/'.str_replace('..', '', $this->param('file')));
